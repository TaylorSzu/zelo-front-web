...
export async function testeCadastroContratante() {
  let driver = await new Builder().forBrowser("chrome").build();
  try {
    await driver.get("http://localhost:5173/cadastro");
    await delay(2000);

    // Aguarde os inputs carregarem
    await driver.wait(
    ...
    await driver.findElement(By.css("select option[value='Paciente']")).click();
    await delay(500);
